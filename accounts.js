document.addEventListener('DOMContentLoaded', function () {
  ...
  if (signupForm) {
    signupForm.addEventListener('submit', function (event) {
      ...
      if (user.length < 3 || !/^[a-z][a-z0-9-]*$/.test(user)) {
        showError(
          signupForm,
          'Gamer tag must be at least 3 characters long. Lowercase letters, numbers, and hyphens are allowed.',
        )
        return
      }
      ...
      if (
        ...
        showError(
          signupForm,
          'Password must be at least 6 characters long and contain at least one uppercase letter and a number.',
        )
        return
      }
      ...
      const takenUser = usersDB.find(
        (u) => u.username.toLowerCase() === user.toLowerCase(),
      )
      if (takenUser) {
        showError(
          signupForm,
          'This gamer tag is used, please choose another one.',
        )
        return
      }
